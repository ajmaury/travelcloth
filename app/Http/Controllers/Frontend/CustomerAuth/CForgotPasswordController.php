...
class CForgotPasswordController extends Controller
{
    public function showForgetPasswordForm()
    {
       return view('frontend.customer.forgotPassword');
    }
    public function submitForgetPasswordForm(Request $request)
    {
        ...
        if($validator->fails()){
            ...
            $where = [['mobile','=',$phone],['account_type','=',0]];
            $orwhere = [['mobile','=',"+91".$phone],['account_type','=',0]];
            $check_customer = Customer::where($where)->orWhere($orwhere)->first();
            if(!empty($check_customer)){
                ...
                $otp_page = view('frontend.customer.forgot_otp')->render();
                return response()->json([
                    'status' => 200,
                    'message' => $otp_page
                ]);
    ...
    public function verify_otp(Request $request)
    {
        ...
        if($validator1->fails()){
            ...
            if($request->input('otp') == $session_data['otp']){
                $otp_page = view('frontend.customer.set_password')->render();
                return response()->json([
                    'status' => 200,
                    'message' => $otp_page
                ]);
    ...
    public function set_password(Request $request)
    {
        ...
        if($validator1->fails()){
            ...
            $customer = Customer::where('mobile', $mobile)->orWhere('mobile', "+91".$mobile);
