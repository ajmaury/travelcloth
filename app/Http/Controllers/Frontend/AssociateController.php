...
class AssociateController extends Controller
{
    ...
    {
        $customer = Customer::find(Auth::guard('associate')->user()->id);
        return view('frontend.associate.profile.personal', compact('customer'));
    }
    public function address()
    {
        ...
        return view('frontend.associate.profile.address', compact('customer', 'countrys', 'states'));
    }
    public function kyc()
    {
        if(trim(Auth::guard('associate')->user()->kyc_status) == "" OR trim(Auth::guard('associate')->user()->kyc_status) == NULL){
            return view('frontend.associate.profile.kyc_form');
        }else{
            $kyc_status = Kyc_status::get();
            $kyc = Kyc::with('customer')->where(['kyc.active_status'=>1,'kyc.customer_id'=>Auth::guard('associate')->user()->id])->first();
            return view('frontend.associate.profile.kyc_view', compact('kyc','kyc_status'));
        }
    }
    ...
    public function postProfile(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $id = Auth::guard('associate')->user()->id;
    ...
    public function deleteKycDoc()
    {
        ...
        if (File::exists($image_path)) {
            ...
            $customer->update($input);
            Toastr::success('File deleted successfully');
            return redirect()->route('associate.kyc');
        } else {
            Toastr::success('File deleted successfully');
            return redirect()->route('associate.kyc');
        }
    ...
    public function postAddress(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $customer->update($input);
            Toastr::success('Updated Successfully');
            return redirect()->route('associate.address');
        }
    }
    ...
    public function changemobile()
    {
        $id = Auth::guard('associate')->user()->id;
        $customer = Customer::find($id);
        return view('frontend.associate.profile.changemobile.index',compact('customer'));
    }
    public function verifyno()
    {
        $mobile = Auth::guard('associate')->user()->mobile;
        $otp_page = view('frontend.associate.profile.verifynumber')->render();
    ...
    {
        if(Session::get('otp')){
            return view('frontend.associate.profile.kyc_update');
        }else{
            ...
            if ($request->input('kyc_type')) {
                ...
                if($request->input('kyc_type') == 'Aadhar Card'){
                    if ($request->hasFile('aadhar_front') AND $request->hasFile('aadhar_back')) {
                        ...
                        Toastr::error('Please Select Aadhar front and back both side.');
                        return redirect()->route('associate.kyc.update');
                    }
                }elseif($request->input('kyc_type') == 'Passport'){
                    if ($request->hasFile('passport_1') AND $request->hasFile('passport_2')) {
                        $passport_1 = $request->file('passport_1');
