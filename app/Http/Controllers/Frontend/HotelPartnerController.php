...
class HotelPartnerController extends Controller
{
    ...
    {
        $customer = Customer::find(Auth::guard('hotelpartner')->user()->id);
        return view('frontend.hotelpartner.profile.personal', compact('customer'));
    }
    public function address()
    {
        ...
        return view('frontend.hotelpartner.profile.address', compact('customer', 'countrys', 'states'));
    }
    public function kyc()
    {
        if(trim(Auth::guard('hotelpartner')->user()->kyc_status) =="" OR trim(Auth::guard('hotelpartner')->user()->kyc_status) == NULL){
            return view('frontend.hotelpartner.profile.kyc_form');
        }else{
            $kyc_status = Kyc_status::get();
            $kyc = Kyc::with('customer')->where(['kyc.active_status'=>1,'kyc.customer_id'=>Auth::guard('hotelpartner')->user()->id])->first();
            return view('frontend.hotelpartner.profile.kyc_view', compact('kyc','kyc_status'));
        }
    }
    ...
    public function postProfile(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $id = Auth::guard('hotelpartner')->user()->id;
    ...
    public function deleteKycDoc()
    {
        ...
        if (File::exists($image_path)) {
            ...
            $customer->update($input);
            Toastr::success('File deleted successfully');
            return redirect()->route('hotelpartner.kyc');
        } else {
            Toastr::success('File deleted successfully');
            return redirect()->route('hotelpartner.kyc');
        }
    ...
    public function postAddress(Request $request)
    {
        ...
        if ($validator->fails()) {
            Toastr::error($validator->errors());
            return redirect()->route('hotelpartner.address');
        } else {
            $input = $request->all();
            $id = Auth::guard('hotelpartner')->user()->id;
            $customer = Customer::find($id);
    ...
    public function changemobile()
    {
        $id = Auth::guard('hotelpartner')->user()->id;
        $customer = Customer::find($id);
        return view('frontend.hotelpartner.profile.changemobile.index',compact('customer'));
    }
    public function verifyno()
    {
        $mobile = Auth::guard('hotelpartner')->user()->mobile;
        $otp_page = view('frontend.hotelpartner.profile.verifynumber')->render();
    ...
    {
        if(Session::get('otp')){
            return view('frontend.hotelpartner.profile.kyc_update');
        }else{
            ...
            if ($request->input('kyc_type')) {
                ...
                if($request->input('kyc_type') == 'GST Certificate'){
                    if ($request->hasFile('gst_certificate')) {
                        ...
                        Toastr::error('Please Select GST Certificate.');
                        return redirect()->route('hotelpartner.kyc.update');
                    }
                }else{
                    if ($request->hasFile('c_incorporation')) {
                        $c_incorporation = $request->file('c_incorporation');
