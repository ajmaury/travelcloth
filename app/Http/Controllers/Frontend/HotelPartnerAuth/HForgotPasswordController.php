...
class HForgotPasswordController extends Controller
{
    public function showForgetPasswordForm()
    {
       return view('frontend.hotelpartner.forgotPassword');
    }
    public function submitForgetPasswordForm(Request $request)
    {
        ...
        if($validator->fails()){
            ...
            $where = [['mobile','=',$phone],['account_type','=',3]];
            $orwhere = [['mobile','=',"+91".$phone],['account_type','=',3]];
            $check_hotelpartner = Customer::where($where)->orWhere($orwhere)->first();
            if(!empty($check_hotelpartner)){
                ...
                $otp_page = view('frontend.hotelpartner.forgot_otp')->render();
                return response()->json([
                    'status' => 200,
                    'message' => $otp_page
                ]);
    ...
    public function verify_otp(Request $request)
    {
        ...
        if($validator1->fails()){
            ...
            if($request->input('otp') == $session_data['otp']){
                $otp_page = view('frontend.hotelpartner.set_password')->render();
                return response()->json([
                    'status' => 200,
                    'message' => $otp_page
                ]);
    ...
    public function set_password(Request $request)
    {
        ...
        if($validator1->fails()){
            ...
            $hotelpartner = Customer::where('mobile', $mobile)->orWhere('mobile', "+91".$mobile);
            $hotelpartner->update(['password'=>$password]);
