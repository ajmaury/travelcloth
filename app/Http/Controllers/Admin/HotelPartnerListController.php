...
class HotelPartnerListController extends Controller
{
            ...
            return Datatables::of($data)
                ...
                ->addColumn('action', function($row){
					if (Gate::check('hotelpartner-view')) {
                        $view = '<a href="javascript:void(0)" onclick=hotelpartnerview("'.$row->id.'") class="custom-edit-btn mr-1">
                                    <i class="fe fe-eye"></i>
                                        '.__('View').'
                                </a>';
                    }else{
    ...
    {
        return view('admin.hotelpartner.create');
    }
    public function store(Request $request)
    {
        ...
        if($validator->fails()){
		    ...
		    return redirect()->route('hotelpartner.sign_up');
        }else{
            $input = request()->all();
            $input['password'] = Hash::make($request->input('password'));
            $input['account_type'] = 3;
            $input['accountId'] = 'TCH'.mt_rand(1000,9999);
            $input['account_status'] = 1;
            $user = Customer::create($input);
            Toastr::success('Created Successfully');
		    return redirect()->route('hotelpartner.index');
        }
    }
    public function view(Request $request)
    {
        $hotelpartner = Customer::addSelect(
            [
        ...
        $kyc_status = Kyc_status::get();
        $viewpage = view('admin.hotelpartner.hotelpartner_view',compact('hotelpartner','kyc','allkyc','kyc_status'))->render();
    ...
    public function change_kyc_status(Request $request)
    {
        ...
        $hotelpartner = Customer::find($request->id);
        $kyc_status = Kyc_status::get();
        $kycstatus = view('admin.hotelpartner.kyc_status',compact('hotelpartner','kyc_status'))->render();
        $kycoption = view('admin.hotelpartner.kyc_option',compact('hotelpartner','kyc_status'))->render();
