...
class AssociateListController extends Controller
{
            ...
            return Datatables::of($data)
                ...
                ->addColumn('action', function($row){
					if (Gate::check('associate-view')) {
                        $view = '<a href="javascript:void(0)" onclick=associateview("'.$row->id.'") class="custom-edit-btn mr-1">
                                    <i class="fe fe-eye"></i>
                                        '.__('View').'
                                </a>';
                    }else{
    ...
    public function store(Request $request)
    {
        ...
        if($validator->fails()){
		    ...
		    return redirect()->route('associate.sign_up');
        }else{
            $input = request()->all();
            $input['password'] = Hash::make($request->input('password'));
            $input['account_type'] = 4;
            $input['accountId'] = 'TCA'.mt_rand(1000,9999);
    ...
    public function view(Request $request)
    {
        ...
        $kyc_status = Kyc_status::get();
        $viewpage = view('admin.associate.associate_view',compact('associate','kyc','allkyc','kyc_status'))->render();
    ...
    public function change_kyc_status(Request $request)
    {
        ...
        $associate = Customer::find($request->id);
        $kyc_status = Kyc_status::get();
        $kycstatus = view('admin.associate.kyc_status',compact('associate','kyc_status'))->render();
        $kycoption = view('admin.associate.kyc_option',compact('associate','kyc_status'))->render();
