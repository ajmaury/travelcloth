    ...
    <div class="row justify-center">
      <div class="col-xl-6 col-lg-7 col-md-9">
        <div class="px-50 py-50 sm:px-20 sm:py-20 bg-white shadow-4 rounded-4 register_output">
          <form method="POST" action="#" id="associate_signup">
            @csrf
            <div class="row y-gap-20">
              <div class="col-12">
                <h1 class="text-22 fw-500">Create An ccount</h1>
                <p class="mt-10">Already have an account? <a href="{{ route('associate.sign_in') }}" class="text-blue-1">Log
                    in</a>
                </p>
              </div>
              <div class="col-12">
...
<script>
  ...
  $(function(){
    $("#associate_signup").submit(function(e){
      e.preventDefault();
    ...
    $(document).delegate("#otpform", "submit", function(e) {
      ...
      $.ajax({
        url: '{{ route('associate.verify_otp') }}',
        method: 'post',
        data: $(this).serialize(),
        dataType: 'json',
        success: function(res){
          console.log(res);
          if($.isEmptyObject(res.error)){
            location.href = "{{ route('associate.sign_in') }}";
            toastr.success(res.message);
          }else{
    ...
    $(document).delegate("#resend_otp","click", function(e){
      e.preventDefault();
      $.ajax({
        url: '{{ route('associate.resend_otp') }}',
        dataType: 'json',
        success: function(res){
          console.log(res);
          if($.isEmptyObject(res.error)){
