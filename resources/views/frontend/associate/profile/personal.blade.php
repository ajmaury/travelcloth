    ...
    <div class="dashboard__content bg-light-2">
        ...
        <div class="py-30 px-30 rounded-4 bg-white shadow-3">
            <div class="tabs -underline-2 js-tabs">
                ...
                <form action="{{ route('associate.profile.post') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="row y-gap-30 items-center">
                        <div class="col-auto">
                            <div class="d-flex ratio ratio-1:1 w-200">
                                <img src="{{ url('/storage/customer_profile/'.$customer->image) }}" alt="image"
                                    id="display_img" style="max-height: 200px !important;
                            position: inherit;" class="img-ratio rounded-4"
                                ...
                                <div class="d-flex justify-end px-10 py-10 h-100 w-1/1 absolute">
                                    <div class="size-40 bg-white rounded-4">
                                        <a href="{{ route('associate.deleteImage') }}"><i
                                                class="icon-trash text-16"></i></a>
                                    </div>
                                </div>
                            </div>
                        </div>
...
$(function(){
    $("#verifymobileotp").submit(function(e){
      e.preventDefault();
      $.ajax({
        url: '{{ route('associate.verify_mobile_otp') }}',
        method: 'post',
        data: $(this).serialize(),
        dataType: 'json',
        success: function(res){
            ...
            if($.isEmptyObject(res.error)){
                ...
                location.href = "{{ route('associate.profile') }}";
            }else{
