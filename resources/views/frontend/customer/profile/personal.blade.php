    ...
    <div class="dashboard__content bg-light-2">
        ...
        <div class="py-30 px-30 rounded-4 bg-white shadow-3">
            <div class="tabs -underline-2 js-tabs">
                ...
                <form action="{{ route('customer.profile.post') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="row y-gap-30 pt-50 items-center">
                        <div class="col-auto">
                            ...
                            position: inherit;" class="img-ratio rounded-4" onerror="this.src='{{ asset('assets/fronted/img/dummy.png') }}';">
                                <div class="d-flex justify-end px-10 py-10 h-100 w-1/1 absolute">
                                    <div class="size-40 bg-white rounded-4"> 
                                        <a href="{{ route('customer.deleteImage') }}"><i class="icon-trash text-16"></i></a>
                                    </div>
                                </div>
                            </div>
                        </div>
...
$(function(){
    $("#verifymobileotp").submit(function(e){
      e.preventDefault();
      $.ajax({
        url: '{{ route('customer.verify_mobile_otp') }}',
        method: 'post',
        data: $(this).serialize(),
        dataType: 'json',
        success: function(res){
            ...
            if($.isEmptyObject(res.error)){
                ...
                location.href = "{{ route('customer.profile') }}";
            }else{
