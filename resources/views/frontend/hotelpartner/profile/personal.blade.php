    ...
    <div class="dashboard__content bg-light-2">
        ...
        <div class="py-30 px-30 rounded-4 bg-white shadow-3">
            <div class="tabs -underline-2 js-tabs">
                ...
                <form action="{{ route('hotelpartner.profile.post') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            position: inherit;" class="img-ratio rounded-4" onerror="this.src='{{ asset('assets/fronted/img/dummy.png') }}';">
                                <div class="d-flex justify-end px-10 py-10 h-100 w-1/1 absolute">
                                    <div class="size-40 bg-white rounded-4"> 
                                        <a href="{{ route('hotelpartner.deleteImage') }}"><i class="icon-trash text-16"></i></a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-xl-9">
                        <div class="row x-gap-20 y-gap-20">
                            ...
                            <div class="col-md-12">
                                <div class="form-input ">
                                    <input type="text" name="hotelName" id="hotelName" @error('hotelName') form-control-error
                                        @enderror" required="required" value="{{$customer->hotelName}}">
...
$(document).delegate(".verifyno", "click", function(e) {
    e.preventDefault();
    $.ajax({
    url: '{{ route('hotelpartner.verifyno') }}',
    dataType: 'json',
...
$(function(){
    $("#verifymobileotp").submit(function(e){
      e.preventDefault();
      $.ajax({
        url: '{{ route('hotelpartner.verify_mobile_otp') }}',
        method: 'post',
        data: $(this).serialize(),
        dataType: 'json',
        success: function(res){
            ...
            if($.isEmptyObject(res.error)){
                ...
                location.href = "{{ route('hotelpartner.profile') }}";
            }else{
