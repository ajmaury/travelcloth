    ...
    <div class="row justify-center">
      <div class="col-xl-6 col-lg-7 col-md-9">
        <div class="px-50 py-50 sm:px-20 sm:py-20 bg-white shadow-4 rounded-4 register_output">
          <form method="POST" action="#" id="hotelpartner_signup">
            @csrf
            <div class="row y-gap-20">
              <div class="col-12">
                <h1 class="text-22 fw-500">Create An ccount</h1>
                <p class="mt-10">Already have an account? <a href="{{ route('hotelpartner.sign_in') }}" class="text-blue-1">Log
                    in</a>
                </p>
              </div>
              <div class="col-12">
                <div class="form-input">
                  ...
                  <label class="lh-1 text-14 text-light-1">Hotel Name</label>
                </div>
                <span class="hotelName_err error"></span>
              </div>
              <div class="col-12">
                <div class="form-input">
...
<script>
  ...
  $(function(){
    $("#hotelpartner_signup").submit(function(e){
      e.preventDefault();
      $("#register_btn").val("Please wait...");
      $.ajax({
        url: '{{ route('hotelpartner.register') }}',
        method: 'post',
        data: $(this).serialize(),
        dataType: 'json',
    ...
    $(document).delegate("#otpform", "submit", function(e) {
      ...
      $.ajax({
        url: '{{ route('hotelpartner.verify_otp') }}',
        method: 'post',
        data: $(this).serialize(),
        dataType: 'json',
        success: function(res){
          console.log(res);
          if($.isEmptyObject(res.error)){
            location.href = "{{ route('hotelpartner.sign_in') }}";
            toastr.success(res.message);
          }else{
    ...
    $(document).delegate("#resend_otp","click", function(e){
      e.preventDefault();
      $.ajax({
        url: '{{ route('hotelpartner.resend_otp') }}',
        dataType: 'json',
        success: function(res){
          console.log(res);
          if($.isEmptyObject(res.error)){
