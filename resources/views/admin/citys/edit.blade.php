							...
							<div class="row">
								<div class="col-md-12">
									<div class="form-group">
										...
										<select type="text" name="state_id" id="state_id" class="form-control @error('state_id') form-control-error @enderror" required="required">
											...
											@foreach ($states as $state)
												<option value="{{$state->id}}" @if ($city->state_id == $state->id) @endif selected>{{$state->state_name}}</option>
											@endforeach	
									...
									<div class="form-group">
										...
										<select type="text" name="zone_id" id="zone_id" class="form-control @error('zone_id') form-control-error @enderror" required="required">

											<option value="">Choose Zone</option>
											@foreach ($zones as $zone)
												<option value="{{$zone->id}}" @if ($city->zone_id == $zone->id) @endif selected>{{$zone->zone_name}}</option>
											@endforeach	
									...
									<div class="form-group">
										...
										<input type="number" name="pincode" id="pincode" class="form-control @error('pincode') form-control-error @enderror" required="required" value="{{$city->pincode}}">
