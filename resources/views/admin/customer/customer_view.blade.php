...
<div class="modal-body">
    <div class="row">
        <div class="col-sm-3">
            <img src="{{ url('/storage/customer_profile/'.$customer->image) }}" alt="image" id="display_img" style="max-height: 135px !important;max-width:170px !important;
                            position: inherit;" class="img-ratio rounded-4"
                onerror="this.src='{{ asset('assets/fronted/img/dummy.png') }}';">
        </div>
        <div class="col-sm-9">
            <p> <strong>Customer ID - </strong> {{ $customer->accountId }}</p>
            <p> <strong>Name - </strong> {{ $customer->fname." ".$customer->lname }}</p>
            <p> <strong>Mobile - </strong> {{ $customer->mobile }}
    ...
    @if (trim($customer->kyc_status) !="" AND trim($customer->kyc_status) != NULL)
    <div class="row">
        <div class="col-sm-12">
            <h4 style="padding-top: 20px;">Kyc Detail - 
                ...
                <span id="kycoption">
                    ...
                    <select id="change_kyc_status">
                        ...
                        <option value="{{ $kyc_sta->status_code }}" {{ $customer->kyc_status == $kyc_sta->status_code ? 'selected':'' }}>{{$kyc_sta->status_name  }}</option>
                        @endforeach
                    </select>
                    @endif
                </span>
        ...
        <tbody>
            <tr>
                ...
                <td id="kyc_status_output">
                    ...
                    @endif
                    @foreach ($kyc_status as $kyc_sta)
                        @if ($customer->kyc_status== $kyc_sta->status_code)
...
<script>
    $(document).delegate("#change_kyc_status", "change", function(e) {
        ...
        $.ajax({
            url: `{{ route('customer.change_kyc_status') }}`,
            type: 'GET',
            data: {
                _token: _token,
                id: id,
