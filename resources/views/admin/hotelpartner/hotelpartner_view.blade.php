<div class="modal-header">
    <h4 class="modal-title">Hotel Partner Information</h4>
    <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
</div>
<div class="modal-body">
    <div class="row">
        <div class="col-sm-3">
            <img src="{{ url('/storage/customer_profile/'.$hotelpartner->image) }}" alt="image" id="display_img" style="max-height: 135px !important;
                            position: inherit;" class="img-ratio rounded-4"
                onerror="this.src='{{ asset('assets/fronted/img/dummy.png') }}';">
        </div>
        <div class="col-sm-9">
            <p> <strong>Hotel Partner ID - </strong> {{ $hotelpartner->accountId }}</p>
            <p> <strong>Name - </strong> {{ $hotelpartner->fname." ".$hotelpartner->lname }}</p>
            <p> <strong>Mobile - </strong> {{ $hotelpartner->mobile }}
                @if ($hotelpartner->mobile_verification_status)
                <span
            ...
            <p> <strong>Email - </strong> {{ $hotelpartner->email }}</p>
            <p> <strong>Hotel Name - </strong> {{ $hotelpartner->hotelName }}</p>
    ...
    <table class="table table-bordered">
        <tbody>
            <tr>
                <th>Country</th>
                <td>{{ $hotelpartner->country }}</td>
                <th>State</th>
                <td>{{ $hotelpartner->state }} </td>
            </tr>
            <tr>
                <th>City</th>
                <td>{{ $hotelpartner->city }}</td>
                <th>Pincode</th>
                <td>{{ $hotelpartner->pincode_id }}</td>
            </tr>
            <tr>
                <th>Address Line 1</th>
                <td>{{ $hotelpartner->address_line1 }}</td>
                <th>Address Line 2</th>
                <td>{{ $hotelpartner->address_line2 }}</td>
            </tr>

        </tbody>
    </table>
    @if (trim($hotelpartner->kyc_status) !="" AND trim($hotelpartner->kyc_status) != NULL)
    <div class="row">
        <div class="col-sm-12">
            <h4 style="padding-top: 20px;">Kyc Detail - 
                <input type="hidden" value="{{ $hotelpartner->id }}" id="customerid">
                <span id="kycoption">
                    @if ($hotelpartner->kyc_status != 1)
                    <select id="change_kyc_status">
                        @foreach ($kyc_status as $kyc_sta)
                        <option value="{{ $kyc_sta->status_code }}" {{ $hotelpartner->kyc_status == $kyc_sta->status_code ? 'selected':'' }}>{{$kyc_sta->status_name  }}</option>
                        @endforeach
                    </select>
                    @endif
                </span>
        ...
        <tbody>
            <tr>
                ...
                <td id="kyc_status_output">
                    ...
                    @endphp
                    @elseif($hotelpartner->kyc_status==2)
                    @php
                    ...
                    @endif
                    @foreach ($kyc_status as $kyc_sta)
                        @if ($hotelpartner->kyc_status== $kyc_sta->status_code)
...
<script>
    $(document).delegate("#change_kyc_status", "change", function(e) {
        ...
        $.ajax({
            url: `{{ route('hotelpartner.change_kyc_status') }}`,
            type: 'GET',
            data: {
                _token: _token,
                id: id,
